...
/**
 * Initializes the Echo/Pusher instance for WebSocket communication
 * @param {Object} config - Configuration for Echo initialization
...
export function initializeEcho(config = {}) {
    ...
    // Create Echo instance with provided config or defaults
    echoInstance = new Echo({
        broadcaster: 'reverb',
        key: config.key || 'hawki2',
        wsHost: config.wsHost || (typeof window !== 'undefined' ? window.location.hostname : ''),
...
export function connectWebSocket(roomSlug, onBroadcasterCallback) {
    ...
    const echo = echoInstance || initializeEcho();
